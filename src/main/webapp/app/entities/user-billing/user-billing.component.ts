...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.userBillingService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/user-billing'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: UserBilling) {
        return item.id;
    }
    registerChangeInUserBillings() {
        this.eventSubscriber = this.eventManager.subscribe('userBillingListModification', (response) => this.loadAll());
    }
