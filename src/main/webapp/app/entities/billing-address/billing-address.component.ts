...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.billingAddressService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/billing-address'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: BillingAddress) {
        return item.id;
    }
    registerChangeInBillingAddresses() {
        this.eventSubscriber = this.eventManager.subscribe('billingAddressListModification', (response) => this.loadAll());
    }
